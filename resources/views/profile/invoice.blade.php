    ...
    <tr>
        <td>
          <p class="font" style="margin-left: 20px;">
           ...
           <strong>Address:</strong> {{ $city }}, {{ $district }}, {{ $state }} <br>
  ...
  <table width="100%">
    ...
    <tbody>
     ...
     @foreach ($orderItems as $orderItem)
      ...
      <tr class="font">
        <td align="center">
            <img style="width:50px;height:50px" src="{{ public_path('uploads/products/thumbnails/' . $orderItem->product->thumbnail)  }}" alt="">
        </td>
        <td align="center">{{ $orderItem->product->name_en }}</td>
        <td align="center">
            
        </td>
        <td align="center">{{ $orderItem->color }}</td>
        <td align="center">{{ $orderItem->product->product_code }}</td>
        <td align="center">{{ $orderItem->quantity }}</td>
        <td align="center">${{ $orderItem->product->discount_price == 0 ? $orderItem->product->selling_price : $orderItem->product->discount_price }}</td>
